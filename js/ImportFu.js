...
var ImportFu =function(param){
	...
	function _setParamIfEmpty(param){
		...
		if(param['success_msg_xid'] == undefined){
			param['success_msg_xid'] = fu_slt2 + '_success_msg';
		}

		if(param['success_msg'] == undefined){
			param['success_msg'] = '読込が終わりました。';
		}
	...
	function _wrapFu(param){
		...
		// 成功メッセージ要素を追加する
		var success_msg_html = "<div id='" + param.success_msg_xid + "' class='text-success' style='display:none'>" + param.success_msg + "</div>";
		fu_wrap.after(success_msg_html);
	...
	function _showSuccess(){
		var elm = jQuery('#' + self.param.success_msg_xid);
		elm.show();
		elm.fadeOut(3000);
